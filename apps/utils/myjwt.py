...
def encoder(payload: Payload) -> str:
    ...
    return jwt.encode(payload=payload.dict(), key=settings.JWT_TOKEN_SECRET_KEY, algorithm=settings.JWT_TOKEN_ALGORITHM, headers=headers)


def decoder(token: str) -> dict:
    try:
        return jwt.decode(token, settings.JWT_TOKEN_SECRET_KEY, algorithms=[settings.JWT_TOKEN_ALGORITHM])
